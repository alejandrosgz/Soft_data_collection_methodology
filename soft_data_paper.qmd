...
Models in hydrology have enriched the knowledge about this discipline, bringing to the scientific community new insights about hydrological questions. However, modelers should ensure that models reproduce the hydrological processes in a realistic way, which is not always the case. Obtaining satisfactory values of performance metrics for one variable (e.g., streamflow) is often considered enough to take a hydrological model as satisfactory, which migth lead to a misrepresentation of the reality behind the model.
...
The presented methodology requires some input data and also some files that have to created by the user. In this section, the input data used in this work and the methodology followed to create the files are described.
...
pcp_grid_points <-  read.csv("used_files/Created_csv/ids_stations_file.csv") 
...
#### Runoff calculation ####
cods <- basins_file$gauging_code # Gauging stations codes
areas <- basins_file$area # Drainage areas
obs_anual <-  list() 
...
#| output: false
   ...
   tmp_grid_points <-  read.csv("used_files/Created_csv/ids_stations_file.csv") %>% arrange(., Basin_ID)  # File with IDs, names, and location of the grid points, and subbasins data  
...
![Peaks selection, groundwater recesssion curve identification and linear adjustment performed for subbasin 4.](figs/peaks_selection_adjustment.png){#fig-recession_curve}
...
The estimation of baseflow contribution has been performed through a more complex and subjective method than the runoff coefficient calculation. The use of a baseflow filter program allows to make the hydrograph components division in an easy way, but also can leads to unrealistic results, since some key factors are not automatically taken into account by the filter and thus need expert knowledge supervision. The filter used, based on two parameters, is relatively simple but also allows to take into account factors such as porosity and transmissivity when separating the hydrograph, therefore being able to reproduce the groundwater systems behavior. As the *alpha* and baseflow index estimation has been performed through a manual process, the obtained values have been saved on the scripts or in a csv file in the case of the baseflow index (*groundwater_results.csv*).
...
Considering the lithology, the highest alpha value would have been expected for the impervious region (IMP), since barely any baseflow response is expected there. However, the obtained value was not significantly different from another regions in the basin (@tbl-reg_alphas). Most probably, consecutive precipitation events in these subbasins and snowmelt processes could be maintaining a baseflow level which is not a result of groundwater contribution, and thus hindering recession constant calculation.
...
#| tbl-cap:
 ...
 read.csv("used_files/Created_csv/groundwater_results.csv") %>% .[,c(2,1,3,4,5,6)]%>% 
...
read.csv("used_files/Created_csv/basins_file.csv") %>% left_join(., data_range, "gauging_code") %>% 
...
![Hydrographs comparisson of subbbasins located in the different geological regions.](figs/hydrographs.png){#fig-hydrographs}
...
gw_file <- read.csv("used_files/Created_csv/groundwater_results.csv") %>% .[,c(2,1,3,4,5,6)]
